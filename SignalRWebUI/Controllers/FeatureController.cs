...
namespace SignalRWebUI.Controllers
{
    public class FeatureController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateFeatureDto createFeatureDto)
        {
            ...
            var response = await client.PostAsync("https://localhost:7044/api/Feature", stringContent);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7044/api/Feature/" + id);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7044/api/Feature/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> Update(UpdateFeatureDto updateFeatureDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7044/api/Feature", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
